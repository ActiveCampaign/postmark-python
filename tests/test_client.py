...
class TestClientModule:
    ...
    async def test_request_success(self):
        ...
        with patch.object(
            ...
            response = await client.request(
                method="GET", endpoint="/test", server_token="test-token"
            )
    ...
    async def test_get_post_put_delete(self):
        ...
        with patch(
            "postmark.models.client.request", new_callable=AsyncMock
        ) as mock_request:
            mock_request.return_value = mock_response
            ...
            mock_request.assert_called_with(
                "GET", "/test", server_token="token", params={"key": "value"}
            )

            # Test POST
            await client.post("/test", "token", json={"data": "value"})
            mock_request.assert_called_with(
                "POST", "/test", server_token="token", json={"data": "value"}
            )

            # Test PUT
            await client.put("/test", "token", json={"data": "value"})
            mock_request.assert_called_with(
                "PUT", "/test", server_token="token", json={"data": "value"}
            )

            # Test DELETE
            # await client.delete("/test", "token")
            # mock_request.assert_called_with(
            #     "DELETE", "/test", server_token="token", json=None
            # )
