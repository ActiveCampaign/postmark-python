...
class TestAccountClient:
    ...
    def test_negative_retries_raises(self):
        with pytest.raises(PostmarkException, match="retries"):
            AccountClient(account_token="test-account-token", retries=-1)

    def test_zero_timeout_raises(self):
        with pytest.raises(PostmarkException, match="timeout"):
            AccountClient(account_token="test-account-token", timeout=0)

    def test_negative_timeout_raises(self):
        with pytest.raises(PostmarkException, match="timeout"):
            AccountClient(account_token="test-account-token", timeout=-1.0)
