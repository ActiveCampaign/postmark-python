...
class TestOutboundMessages:
    ...
    async def test_find_messages_success(self, mock_response_data):
        ...
        with patch('postmark.models.messages.client.get', new_callable=AsyncMock) as mock_get:
    ...
    async def test_find_all_pagination(self):
        ...
        response1_data = {
            "TotalCount": 750,
            "Messages": [{"MessageID": f"msg-{i}", "Subject": f"Subject {i}", 
                         "From": "test@example.com", "To": [{"Email": "user@example.com"}],
                         "Recipients": ["user@example.com"], "ReceivedAt": "2024-01-15T10:00:00Z",
                         "Status": "Sent", "TrackOpens": True, "TrackLinks": "None",
                         "MessageStream": "outbound", "Cc": [], "Bcc": [],
                         "Attachments": [], "Metadata": {}, "Sandboxed": False}
                        for i in range(500)]
        }
    ...
    async def test_find_by_id(self):
        ...
        with patch('postmark.models.messages.client.get', new_callable=AsyncMock) as mock_get:
            ...
            mock_get.assert_called_once_with(
                "/messages/outbound/msg-123/details",
                server_token="test-token"
            )
...
class TestEmailValidation:
    ...
    def test_validate_formatted_email_valid(self):
        ...
        # Formatted email
        assert validate_formatted_email('"John Doe" <john@example.com>') == '"John Doe" <john@example.com>'
        
        # Email with no quotes
        assert validate_formatted_email('Jane Doe <jane@example.com>') == 'Jane Doe <jane@example.com>'
