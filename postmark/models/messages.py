...
from pydantic import BaseModel, ConfigDict, Field, EmailStr, ValidationError, BeforeValidator, TypeAdapter
...
class GeoInfo(BaseModel):
    ...
    ip: Optional[str] = Field(None, alias="IP")  # Keep as str since PostMark returns it as string
...
class Outbound(BaseModel):
    ...
    attachments: List[Union[str, Attachment]] = Field(default_factory=list, alias="Attachments")
    ...
    async def find(
        ...
        response = await client.get("/messages/outbound", server_token=server_token, params=params)
    ...
    async def get(cls, message_id: str, server_token: str) -> "OutboundMessageDetails":
        ...
        response = await client.get(f"/messages/outbound/{message_id}/details", server_token=server_token)
...
class OutboundMessageDetails(Outbound):
    ...
    message_events: List[MessageEvent] = Field(default_factory=list, alias="MessageEvents")
...
class InboundMessageSummary(BaseModel):
    ...
    original_recipient: str = Field(alias="OriginalRecipient")  # Keep as str - might contain hash
    subject: str = Field(alias="Subject")
...
class MessageClick(BaseModel):
    ...
    platform: Optional[Platform] = Field(None, alias="Platform")
    user_agent: str = Field(alias="UserAgent")
    original_link: str = Field(alias="OriginalLink")  # Keep as str - might be relative URL
    geo: Optional[GeoInfo] = Field(None, alias="Geo")
    message_id: str = Field(alias="MessageID")
    message_stream: str = Field(alias="MessageStream")
    received_at: datetime = Field(alias="ReceivedAt")
