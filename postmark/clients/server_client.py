...
from ..utils.timing import calc_request_duration, start_timer
...
class ServerClient:
    ...
    def __init__(
        ...
        if retries < 0:
            raise PostmarkException("retries must be a non-negative integer.")
        if timeout <= 0:
            raise PostmarkException("timeout must be a positive number.")
        ...
        self._http_client = httpx.AsyncClient(
            ...
            headers={
                ...
                "X-Postmark-Client-Version": __version__,
            ...
            with attempt:
                try:
                    ...
                    response = await self._http_client.request(
                        method,
                        endpoint,
                        headers={"X-Postmark-Correlation-Id": str(uuid.uuid4())},
                        **kwargs,
                    )
                    response.raise_for_status()
                    duration_ms = calc_request_duration(start)
