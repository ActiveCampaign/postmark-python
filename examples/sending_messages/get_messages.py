...
async def main():
    # --- List (one page) ---
    messages, total = await client.email.list(count=10)
    ...
    async for msg in client.email.stream(max_messages=50):
    ...
    if messages:
        ...
        detail = await client.email.get(messages[0].message_id)
